...
void Graficador::graficarImagen()
{
    ...
    if(controlPseudocolorear)
    {
        lut.leerArchivoLUT(auxDirectorioLUT+listadoLUT[indiceLUT]);
        imagen = lut.pseudocolorear2(imagenOriginal);
        controlPseudocolorear = false;
    }
    glPushMatrix();
    {
...
void Graficador::graficarHistograma()
{
    ...
    {
        for(unsigned int i = 0; i<vectorRepeticionesValoresR.size(); i++)
        {
        ...
        for(unsigned int i = 0; i<vectorRepeticionesValoresG.size(); i++)
        {
            glBegin(GL_LINE_STRIP);
            {
        ...
        for(unsigned int i = 0; i<vectorRepeticionesValoresB.size(); i++)
        {
            glBegin(GL_LINE_STRIP);
            {
...
void Graficador::keyPressEvent(QKeyEvent *pEvent)
{
    ...
    {
        if(unsigned (indiceImagen) < listadoDeImagenes.size()-1)
        {
            indiceImagen++;
            setImagen(listadoDeImagenes[indiceImagen], auxDirectorio);
        }
    ...
    {
        puntero_filtro = new FiltroPasaBajos;
    }

    if(controlPresionado && mPresionada)
    {
        puntero_filtro = new FiltroMediana;
    }

    if(controlPresionado && aPresionada)
    {
        puntero_filtro = new FiltroPasaAltos;
    }

    if(puntero_filtro != NULL)
    {
        puntero_filtro->procesarImagen(imagen);
        delete puntero_filtro;
        puntero_filtro = NULL;
    }
