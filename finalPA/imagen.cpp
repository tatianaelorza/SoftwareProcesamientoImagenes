...
void Imagen::generarMatriz(int pAltoImagen, int pAnchoImagen)
{
    ...
    matrizImagen.resize(getAltoImagen());

    for(int i = 0; i<getAltoImagen(); i++)
    {
        matrizImagen[i].resize(getAnchoImagen());
    }
}
...
{
    return matrizImagen[pFila][pColumna];
}
...
{
    matrizImagen[pFila][pColumna].setR(r);
    matrizImagen[pFila][pColumna].setG(g);
    matrizImagen[pFila][pColumna].setB(b);
}
float Imagen::getValorR(int pFila, int pColumna)
{
    return (matrizImagen[pFila][pColumna].getRGB()[0]);
}
float Imagen::getValorG(int pFila, int pColumna)
{
    return (matrizImagen[pFila][pColumna].getRGB()[1]);
}
float Imagen::getValorB(int pFila, int pColumna)
{
    return (matrizImagen[pFila][pColumna].getRGB()[2]);
}
void Imagen::cambiarBrillo(float pCantBrillo)
{
    for(int i = 0; i<getAltoImagen(); i++)
    {
        ...
        {
            float auxR = getValorR(i, j)+pCantBrillo;
            float auxG = getValorG(i, j)+pCantBrillo;
            float auxB = getValorB(i, j)+pCantBrillo;
...
void Imagen::negativizarImagen()
{
    ...
    for(int i = 0; i<getAltoImagen(); i++)
    {
        for(int j = 0; j<getAnchoImagen(); j++)
        {
            r = getMaxRangoDinamico() - getValorR(i, j);
            g = getMaxRangoDinamico() - getValorG(i, j);
            b = getMaxRangoDinamico() - getValorB(i, j);
...
void Imagen::controlMaxRangoDinamico(int pFila, int pColumna)
{
    ...
    R = getValorR(pFila, pColumna);
    G = getValorG(pFila, pColumna);
    B = getValorB(pFila, pColumna);
